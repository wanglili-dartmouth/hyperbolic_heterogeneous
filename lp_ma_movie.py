...
def walk_generation(G, path_length, heterg_dictionary, m, start):
    path = []   
    path.append(start)
    no_next_types = 0    
...
def generate_walks(G, num_walks, walk_length, m, heterg_dictionary):
    ...
    for cnt in tqdm(range(num_walks)):
        ...
        for node in nodes:
            just_walks = walk_generation(G, walk_length, heterg_dictionary, m, start=node)
            #print(just_walks)
...
def main():
    ...
    print('a:',len(nodes_list_a))
    print('d:',len(nodes_list_c))
    print('m:',len(nodes_list_p))
    test_edges=[]
    total_edges = list(graph.subgraph(nodes_list_p+nodes_list_a).edges())
    non_edges=[]
    for p in nodes_list_p:
        for c in nodes_list_a:
            if not graph.has_edge(p,c):
                non_edges.append((p,c))
    print('total_edges:',len(total_edges))
    print('non_edges:',len(non_edges))
    remove_edges= int (len(total_edges)*0.20)
    print(nx.number_connected_components(graph))
    tmp=nx.number_connected_components(graph)
    while remove_edges>0:
        e=random.choice(total_edges)
        graph.remove_edge(e[0],e[1])
        if nx.number_connected_components(graph)>tmp:
            graph.add_edge(e[0],e[1])
        else:
            total_edges.remove(e)
            test_edges.append(e)
            remove_edges-=1
    print(nx.number_connected_components(graph))
    test_non_edges=random.sample(non_edges,len(test_edges))
    print("len pos: ",len(test_edges))
    print("len neg: ",len(test_non_edges))
    print("--------------------------------")
    ...
    print(walks[0])
    print(test_edges)
    test_edges=[ (Name[edge[0]],Name[edge[1]]) for edge in test_edges]
    test_non_edges=[ (Name[edge[0]],Name[edge[1]]) for edge in test_non_edges]
